...
public class HoleToUnderground : GenerationStep
{
    ...
    protected override IEnumerator<object> OnPerform(GenerationContext context)
    {
        ...
        // dig out a corridor to the caves
        var success = false;
        var tries = 0;
        ...
        while (!success && tries < 100)
        {
            ...
            // Reject if tunnel isn't a good size
            if (tunnel.Count < 9 || tunnel.Count > 18)
                continue;

            success = true;

            // destroy a circle around the tunnel leading up to the hole
            var tunnelHalfway = tunnel[tunnel.Count / 2] + 1;
            var radius = (tunnel.Count / 2) + 1;

            var pointsInCircle = wallsFloors.CircleCoveringPoints(tunnelHalfway, radius);

            var features = context.GetFirstOrNew(() => new ItemList<GameObjectTypePosition<FeatureType>>(), MapGenerator.FeaturesTag);

            // randomise wall, floor and rubble
            foreach (var point in pointsInCircle)
            {
                // floors = true
                var makeFloor = RNG.NextBool();

                wallsFloors[point] = makeFloor;

                if (makeFloor)
                {
                    wallsFloorTypes[point] = floorTypeRock;

                    if (RNG.NextBool())
                        features.Add(new GameObjectTypePosition<FeatureType>(FeatureType.RubbleType, point), Name);
                }
            ...
            var bounds = new Rectangle(
                new Point(tunnelHalfway.X - radius, tunnelHalfway.Y - radius),
                new Point(tunnelHalfway.X + radius, tunnelHalfway.Y + radius));

            bounds = Rectangle.GetIntersection(bounds, wallsFloors.Bounds());

            var diggedFloors = bounds.WiggleDig(tunnel[0], tunnel[^1], RNG);

            foreach (var diggedFloor in diggedFloors)
            {
                wallsFloors[diggedFloor] = true;
                wallsFloorTypes[diggedFloor] = floorTypeRock;
            }
